...
export const getPTOEntries = (orgId: string, year?: number) =>
  isDemo() ? mock.getPTOEntries(year) : http.getPTOEntries(orgId, year);
export const getPTOConfig = (orgId: string, year?: number) =>
  isDemo() ? mock.getPTOConfig(year) : http.getPTOConfig(orgId, year);
export const createPTOEntry = (orgId: string, data: Parameters<typeof mock.createPTOEntry>[0]) =>
  isDemo() ? mock.createPTOEntry(data) : http.createPTOEntry(orgId, data);
...
export const updatePTOConfig = (orgId: string, updates: Parameters<typeof mock.updatePTOConfig>[0]) =>
  isDemo() ? mock.updatePTOConfig(updates) : http.updatePTOConfig(orgId, updates);

...
export function getPTOEntries(orgId: string, year?: number): Promise<PTOEntry[]> {
  const qs = year != null ? `?year=${year}` : '';
  return apiFetch(`/api/orgs/${orgId}/pto/entries${qs}`);
}

export function getPTOConfig(orgId: string, year?: number): Promise<PTOConfig> {
  const qs = year != null ? `?year=${year}` : '';
...
export function updatePTOConfig(orgId: string, updates: Partial<PTOConfig>): Promise<PTOConfig> {
